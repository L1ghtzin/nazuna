...
export default {
  ...
  handle: async ({ reply, q, isGroup, MESSAGES, optimizer, groupData, groupFile }) => {
    // Lógica do comando
...
3. **Persistência de Dados Segura:** NUNCA use `fs.readFileSync` ou `fs.writeFileSync` diretamente em arquivos JSON de banco de dados. Use:
   - `optimizer.saveJsonWithCache()` para gravações seguras e síncronas.
   - `writeJsonFileAsync` apenas para tarefas em background que não dependam da ordem cronológica exata do event loop (Cuidado com Race Conditions!).
